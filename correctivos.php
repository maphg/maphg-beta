...
if ($idDestinoT == 10) {
    if ($idSeccionSesion != 0) {
        $query = "SELECT * FROM t_mc "
                ...
                . "activo = 1 ORDER BY id_destino";
    } else {
        $query = "SELECT * FROM t_mc "
                . "WHERE (fecha_creacion >= '$mesAtrasMysql' AND fecha_creacion <= '$hoy') AND "
                . "id_seccion = $idSeccionSesion AND activo = 1 ORDER BY id_destino";
    }
} else {
    if ($idSeccionSesion != 0) {
        $query = "SELECT * FROM t_mc "
                . "WHERE (fecha_creacion >= '$mesAtrasMysql' AND fecha_creacion <= '$hoy') AND "
                . "id_destino = $idDestinoT AND activo = 1 ORDER BY id_destino";
    } else {
        $query = "SELECT * FROM t_mc "
                . "WHERE (fecha_creacion >= '$mesAtrasMysql' AND fecha_creacion <= '$hoy') AND "
                . "id_destino = $idDestinoT AND id_seccion = $idSeccionSesion AND activo = 1 ORDER BY id_destino";
    }
}
                            ...
                            <div class="column">
                                <div class="container">
                                    <div class="navbar-end">
                                        ...
                                        <a class="navbar-item is-active" href="correctivos.php">
                                            Correctivo
                                        </a>
                    ...
                    <div class="columns is-centered px-4 has-background-dark my-4 pt-1">
                        ...
                        <div class="column has-text-centered">
                            <div class="columns is-centered">
                                ...
                                <div class="column">
                                    <button class="button is-info is-small" onclick="buscarMC();">
                                        Buscar
                                    </button>
                                </div>
                            </div>
                ...
                <section>
                    ...
                    <div class="columns">
                        <div class="column is-4 is-one-third">
                            ...
                            <div class="columns is-centered px-3">
                                <div class="column is-12">
                                    <table id="tablaUsuarios" class="table is-size-7" style="width: 100%">
                                        <thead>
                                            <tr>
                                                <th style="display:none;">ID</th>
                                                <th style="width: 60%">Nombre</th>
                                                <th><abbr title="Pendientes">Pendientes</abbr></th>
                                                <th><abbr title="Solucionados">Solucionados</abbr></th>
                                            ...
                                            try {
                                                ...
                                                if ($conn->filasConsultadas > 0) {
                                                    foreach ($resp as $dts) {
                                                        ...
                                                        $query = "SELECT * FROM t_mc WHERE responsable = $idUsuarioR AND status = 'N' AND activo = 1";
                                                        try {
                                                            $resp = $conn->obtDatos($query);
                                                            $totalPendientes = $conn->filasConsultadas;
                                                        } catch (Exception $ex) {
                                                            echo $ex;
                                                        }

                                                        $query = "SELECT * FROM t_mc WHERE responsable = $idUsuarioR AND status = 'F' AND activo = 1";
                                                        try {
                                                            $resp = $conn->obtDatos($query);
                                                            $totalSolucionadas = $conn->filasConsultadas;
                                                        } catch (Exception $ex) {
                                                            echo $ex;
                                                        }

                                                        echo "<tr class=\"manita\">"
                                                        . "<td style=\"display:none;\">$idUsuarioR</td>"
                                                        . "<td>$nombre $apellido</td>"
                                                        . "<td>$totalPendientes</td>"
                                                        . "<td>$totalSolucionadas</td>"
                                                        . "</tr>";
                                                    }
                                                }
                                            } catch (Exception $ex) {
                        ...
                        <div class="column is-8">
                            <h5 class="subtitle is-5">
                                Correctivos Complejo
                            </h5>

                            <div class="columns is-centered container-scroll">
                                <div class="column is-mobile px-4">
        ...
        $(document).ready(function () {
            ...
            usuarios
                    .on('select', function (e, dt, type, indexes) {
                        ...
                        var idResponsable = rowData[0][0];
                        location.href = "mis-pendientes.php?idResponsable=" + idResponsable;
                    });
