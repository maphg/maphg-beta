...
if ($xlsIdDestino != 10) {
    ...
    $query = "SELECT
	reporte_status_proyecto.status, reporte_status_proyecto.fecha_inicio,
	t_proyectos.id,
	t_proyectos_planaccion.actividad, t_proyectos_planaccion.fecha_creacion,
	t_colaboradores.nombre, t_colaboradores.apellido
	...
	WHERE t_proyectos_planaccion.status='N' AND t_proyectos_planaccion.activo=1 AND reporte_status_proyecto.status !='status_solucionado' AND reporte_status_proyecto !=''";
...
while($row_P = mysqli_fetch_array($result_P)){
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('A'.$fila,'wqewqewq');
    $objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, 'wqewqewq');
    $objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, 'wqewqewq');
    //$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $destino);
    //$objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $seccion);
    //$objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, $subseccion);
    //$objPHPExcel->getActiveSheet()->setCellValue('G'.$fila, $departamentos);
