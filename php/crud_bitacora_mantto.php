...
if (isset($_POST['action'])) {
    ...
    if ($action == "cantidadTurno") {
        ...
        if($row_1 = mysqli_fetch_array($result_1)){
			...
			if($idDestino != 10){$cantidadTotal = $row_1['total_turno'];}else{$cantidadTotal = $row_1['sum(total_turno)'];}
        ...
        if($row_2 = mysqli_fetch_array($result_2)){
			...
			if($idDestino != 10){$cantidadTotal = $row_2['total_turno'];}else{$cantidadTotal = $row_2['sum(total_turno)'];}
        ...
        if($row_3 = mysqli_fetch_array($result_3)){
			...
			if($idDestino != 10){$cantidadTotal = $row_3['total_turno'];}else{$cantidadTotal = $row_3['sum(total_turno)'];}
    ...
    if ($action == "MPMCPROYECTOS") {
        ...
        if ($zona == "ZI") {
            ...
            $zonaFiltro = "AND reporte_status_proyecto.id_seccion IN(11, 24, 8, 9, 10, 1)";
		...
		while($row_MC_trabajare = mysqli_fetch_array($result_MC_trabajare)){
            ...
            $bitacoraMC .=
            "<div class=\"flex justify-left items-center w-full bg-red-200 rounded mb-2 text-red-700 cursor-pointer py-2 text-xs px-1\" onclick=\"consultaMPMCPROYECTOS($id, '$seccion', '$subseccion', '$descripcion', '$comentario_mc', '$tag_finalizado', '$tag_status'); toggleModal('modalMCMPProyectos');\">"
            . " $tag_finalizado $tag_status <h1 class=\"\">  $seccion</h1>"
            . "<P class=\"font-black mx-1\">/</P>"
            . "<h1 class=\"\">$subseccion</h1>"
            . "<P class=\"font-black mx-1\">/</P>"
