...
if (isset($_GET['listaIdF']) and isset($_GET['listaIdT']) and isset($_GET['generadoPor'])) {
    ...
    //Correctivos Generales Fallas.
    $queryF = "SELECT t_mc.id, t_mc.creado_por, c_destinos.destino, c_secciones.seccion, c_subsecciones.grupo, t_equipos.equipo, t_mc.actividad, t_colaboradores.nombre, t_colaboradores.apellido 
    FROM t_mc 
    ...
    $queryT = "SELECT t_mp_np.id, t_mp_np.id_usuario, c_destinos.destino, c_secciones.seccion, c_subsecciones.grupo, t_equipos.equipo, t_mp_np.titulo,  t_mp_np.titulo, t_colaboradores.nombre, t_colaboradores.apellido
    FROM t_mp_np
    ...
    if ($resultF = mysqli_query($conn_2020, $queryF) and $resulT = mysqli_query($conn_2020, $queryT)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('I1', 'Tipo Pendiente');
        ...
        while ($row = mysqli_fetch_array($resultF)) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('I' . $fila, 'Fallas');
        ...
        while ($rowT = mysqli_fetch_array($resulT)) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('I' . $fila, 'Tareas');
