...
if (isset($_GET['action'])) {
    ...
    if ($action == "obtenerDespieceEquipoX") {
        ...
        $query = "SELECT id, equipo, local_equipo, status, jerarquia
        FROM t_equipos_america
    ...
    #OBTIENE TODOS LOS EQUIPO POR DESTINO->SECCION->SUBSECCION
    if ($action == "obtenerDespieceEquipoZ") {
        $idEquipo = $_GET['idEquipo'];
        $array = array();
        $contador = 0;
    ...
    if ($action == "obtenerDespieceEquipo") {
        ...
        $query = "SELECT id, equipo, local_equipo, status, jerarquia
        FROM t_equipos_america
        WHERE id_equipo_principal = $idEquipo and jerarquia = 'SECUNDARIO' and activo = 1 and status IN('OPERATIVO', 'TALLER')";
        if ($resultEquipo = mysqli_query($conn_2020, $query)) {
            foreach ($resultEquipo as $x) {
                $idEquipo = $x['id'];
                $equipo = $x['equipo'];
